...
def main(file_path, file_uuid):
    ...
    for rule in rules:
        ...
        if exitstatus != 0:
            ...
            failed = True
            continue
        # fmt/101 is XML - we want to collect and package any XML output, while allowing other commands
        # to execute without actually collecting their output in the event that they
        # are writing their output to disk.
        # FPCommandOutput can have multiple rows for a given file, distinguished by the rule that produced it.
        if rule.command.output_format and rule.command.output_format.pronom_id == 'fmt/101':
            ...
            # If the output isn't XML, print the stdout to the screen so it can be monitored by the operator
            print(stdout)
