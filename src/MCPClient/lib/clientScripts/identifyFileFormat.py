...
def save_idtool(file_, value):
    """
    Saves the chosen ID tool's UUID in a unit variable, which allows it to be refetched by a later chain.
    This is necessary in order to allow post-extraction identification to work.
    The replacement dict will be saved to the special 'replacementDict' unit variable, which will be
    transformed back into a passVar when a new chain in the same unit is begun.
    """
    # The unit_uuid foreign key can point to a transfer or SIP, and this tool runs in both.
    # Check the SIP first - if it hasn't been assigned yet, then this is being run during the transfer.
    try:
        unit = file_.sip
    except:
        unit = file_.transfer
    ...
    UnitVariable.objects.create(unit_uuid=unit.pk, variable='replacementDict', variable_value=str(rd))
